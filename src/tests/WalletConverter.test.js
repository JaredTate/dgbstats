...
describe('Wallet Converter — pure helpers', () => {
  ...
  it('readApplicationId reads testnet25 (RC41/RC42/RC43) id', () => {
  ...
  it('APPLICATION_IDS.testnet25 matches the RC41/RC42/RC43 pchMessageStart (FE C5 B8 E6)', () => {
  ...
  it('patchApplicationId converts a testnet24 wallet to RC43 testnet25', () => {
    const buf = makeFakeSqlite(0xFEC4B7E5);
  ...
  it('testnet24 → testnet25 round-trip preserves all other bytes', () => {
...
describe('Wallet Converter — real fixture files', () => {
  ...
  it('real testnet21 fixture → testnet25 writes RC43 magic at offset 68-71', () => {
    const buf = loadFixture('wallet-testnet21.dat');
    const patched = patchApplicationId(buf, APPLICATION_IDS.testnet25);
    const dv = new DataView(patched);
    expect(dv.getUint32(68, false)).toBe(0xFEC5B8E6);
